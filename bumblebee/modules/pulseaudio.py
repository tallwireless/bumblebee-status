...
class Module(bumblebee.engine.Module):
    ...
    def volume(self, widget):
        if self._failed == True:
            logging.debug("output: n/a")
            return "n/a"
        if int(self._mono) > 0:
            logging.debug("output: mono {}%".format(self._mono))
            return "{}%".format(self._mono)
        elif self._left == self._right:
            logging.debug("output: {}%".format(self._left))
            return "{}%".format(self._left)
        else:
            logging.debug("output lr: {}/{}".format(self._left, self._right))
